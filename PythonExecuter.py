...
def start():
    global main_window, executor_window, close_window_ref, settings_window, password_window
    ...
    def open_settings():
        global settings_window, password_window
        ...
        def check_password(event=None):
            ...
            if pass_entry.get() == "admin123":
                ...
                error_label.config(text="❌ Неверный пароль!", fg="red")
        ...
        password_window.configure(bg=settings['bg_color'])
        password_window.title("Введите пароль")

        create_custom_titlebar(password_window, "🔐 Доступ к настройкам", settings['title_bg'])

        main_frame = tk.Frame(password_window, bg=settings['bg_color'])
        main_frame.pack(fill="both", expand=True, padx=20, pady=20)

        tk.Label(main_frame, text="Введите пароль для доступа к настройкам:",
                 bg=settings['bg_color'], fg=settings['text_color'],
                 font=("Segoe UI", 10)).pack(pady=10)

        pass_entry = tk.Entry(main_frame, width=25, show="•",
                              font=("Segoe UI", 12), bg="#2c3136", fg="white",
                              insertbackground='white')
        pass_entry.pack(pady=10)
        pass_entry.bind("<Return>", check_password)
        pass_entry.focus()

        error_label = tk.Label(main_frame, text="", bg=settings['bg_color'])
        error_label.pack()

        btn_frame = tk.Frame(main_frame, bg=settings['bg_color'])
        btn_frame.pack(pady=10)

        tk.Button(btn_frame, text="Войти", command=check_password,
                  bg=settings['button_bg'], fg=settings['button_fg'],
                  font=("Segoe UI", 10), padx=20).pack(side="left", padx=5)

        tk.Button(btn_frame, text="Отмена", command=lambda: password_window.destroy(),
                  bg="#c0392b", fg="white",
                  font=("Segoe UI", 10), padx=20).pack(side="left", padx=5)
    ...
    def open_executor():
        ...
        main_frame = tk.Frame(exemenu, bg=settings['bg_color'])
        main_frame.pack(fill="both", expand=True)

        button_frame = tk.Frame(main_frame, bg=settings['bg_color'])
        button_frame.pack(pady=5)

        btn = tk.Button(button_frame, text="▶", command=run_code,
                        bg=settings['button_bg'], fg=settings['button_fg'],
                        font=('Arial', 10), padx=10)
        btn.pack(side="left", padx=5)

        btn2 = tk.Button(button_frame, text="Clear", command=clear_output,
                         bg=settings['button_bg'], fg=settings['button_fg'],
                         font=('Arial', 10), padx=10)
        btn2.pack(side="left", padx=5)
        ...
        txt_label = tk.Label(main_frame, text=" Code Editor:",
                             font=("Arial", 10),
                             bg=settings['bg_color'], fg=settings['text_color'])
